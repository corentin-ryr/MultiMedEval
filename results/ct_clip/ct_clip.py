...
class CTCLIP(nn.Module):
    ...
    def forward(
        ...
        if exists(aug_text):
            ...
            assert all((t.shape == text.shape for t in aug_text))
        ...
        if exists(aug_image):
            ...
            assert all((i.shape == image.shape for i in aug_image))
        ...
        if self.decoupled_contrastive_learning:
            ...
            text_to_image_exp, image_to_text_exp = (
                t.masked_fill(pos_mask, 0.0)
                for t in (text_to_image_exp, image_to_text_exp)
            )
