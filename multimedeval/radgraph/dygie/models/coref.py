...
from multimedeval.radgraph.allennlp.modules import FeedForward
...
from multimedeval.radgraph.allennlp_models.coref.metrics.conll_coref_scores import (
    ConllCorefScores,
)
from multimedeval.radgraph.allennlp_models.coref.metrics.mention_recall import (
    MentionRecall,
)

from multimedeval.radgraph.dygie.models import shared
from multimedeval.radgraph.dygie.models.entity_beam_pruner import Pruner
...
class CorefResolver(Model):
    ...
    def __init__(
        ...
        self._f_network = FeedForward(
            input_dim=2 * span_emb_dim,
            num_layers=1,
    ...
    def update_spans(self, output_dict, span_embeddings_batched, indices):
        ...
        for doc_key in output_dict:
            ...
            for ix, el in enumerate(output_dict[doc_key]["top_span_indices"].view(-1)):
                ...
                # embedding matrix.
                row_ix = (
                    span_ix[el] // span_embeddings_batched.shape[1] + offsets[doc_key]
                )
    ...
    def coref_propagation_doc(self, output_dict):
        ...
        for t in range(self.coref_prop):
            ...
            antecedent_distribution = self.antecedent_softmax(coreference_scores)[
                :, :, 1:
            ]
            ...
            if antecedent_indices.shape[0] == antecedent_indices.shape[1]:
                selected_top_span_embs = util.batched_index_select(
                    top_span_emb_repeated, antecedent_indices
                ).unsqueeze(0)
                entity_embs = (
                    (
                        selected_top_span_embs.permute([3, 0, 1, 2])
                ...
                ant_var1 = (
                    antecedent_indices.unsqueeze(0)
                    .unsqueeze(-1)
                    .repeat(1, 1, 1, top_span_embeddings.shape[-1])
                )
                ...
                entity_embs = (
                    (
                        torch.gather(top_var1, 2, ant_var1).permute([3, 0, 1, 2])
                        * antecedent_distribution
                    )
                    .permute([1, 2, 3, 0])
                    .sum(dim=2)
            ...
            top_span_embeddings = (
                f_weights * top_span_embeddings + (1.0 - f_weights) * entity_embs
            )

            coreference_scores = self.get_coref_scores(
                top_span_embeddings,
                self._mention_pruner._scorer(top_span_embeddings),
                output_dict["antecedent_indices"],
                output_dict["valid_antecedent_offsets"],
                output_dict["valid_antecedent_log_mask"],
            )
        ...
        for key in uniq_keys:
            ...
            if sentence_lengths[ix].sum().item() > 1:
                output_docs[key] = self._compute_representations_doc(
                    spans_batched[ix],
                    span_mask_batched[ix],
                    span_embeddings_batched[ix],
                    sentence_lengths[ix],
                    ix,
                    coref_labels,
                    doc_metadata,
    ...
    def _compute_representations_doc(
        ...
        span_ix = (
            span_mask_batched.view(-1).nonzero(as_tuple=False).squeeze()
        )  # Indices of the spans to keep.
    ...
    def predict_labels_doc(self, output_dict):
        ...
        if coref_labels is not None:
            ...
            pruned_gold_labels = util.batched_index_select(
                coref_labels.unsqueeze(-1), top_span_indices, flat_top_span_indices
            )
    ...
    def _compute_antecedent_gold_labels(
        top_coref_labels: torch.IntTensor, antecedent_labels: torch.IntTensor
    ):
        """
    ...
    def _flatten_spans(
        self, spans_batched, span_ix, span_embeddings_batched, sentence_lengths
    ):
        """
        ...
        sentence_offset = (
            shared.cumsum_shifted(sentence_lengths).unsqueeze(1).unsqueeze(2)
        )
