...
from multimedeval.radgraph.dygie.models.shared import (
    fields_to_batches,
    batches_to_fields,
)
import copy
...
class Document:
    ...
    def to_json(self):
        ...
        if self.predicted_clusters is not None:
            res["predicted_clusters"] = [
                cluster.to_json() for cluster in self.predicted_clusters
            ]
        if self.event_clusters is not None:
            res["event_clusters"] = [
                cluster.to_json() for cluster in self.event_clusters
            ]
        if self.predicted_event_clusters is not None:
            res["predicted_event_clusters"] = [
                cluster.to_json() for cluster in self.predicted_event_clusters
            ]
...
class EventsBase(ABC):
    def __init__(self, events_json, sentence, sentence_offsets=False):
        self.event_list = [
            self.event_constructor(this_event, sentence, sentence_offsets)
