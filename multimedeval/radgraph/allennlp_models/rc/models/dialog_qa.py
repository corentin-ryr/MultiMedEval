...
from multimedeval.radgraph.allennlp.common.checks import check_dimensions_match
from multimedeval.radgraph.allennlp.data import Vocabulary
from multimedeval.radgraph.allennlp.models.model import Model
from multimedeval.radgraph.allennlp.modules import (
    Seq2SeqEncoder,
    TimeDistributed,
    TextFieldEmbedder,
)
...
from multimedeval.radgraph.allennlp.modules.matrix_attention.linear_matrix_attention import (
    LinearMatrixAttention,
)
from multimedeval.radgraph.allennlp.nn import InitializerApplicator, util
from multimedeval.radgraph.allennlp.training.metrics import (
    Average,
    BooleanAccuracy,
    CategoricalAccuracy,
)

from multimedeval.radgraph.allennlp_models.rc.tools import squad
from multimedeval.radgraph.allennlp_models.rc.models.utils import (
...
class DialogQA(Model):
    ...
    def forward(  # type: ignore
        ...
        self_attention_vecs = torch.cat(
            [self_attention_vecs, residual_layer, residual_layer * self_attention_vecs],
            dim=-1,
        )
        ...
        start_rep = self._span_start_encoder(
            final_merged_passage, repeated_passage_mask
        )
        ...
        if span_start is not None:
            ...
            self._span_start_accuracy(
                span_start_logits, span_start.view(-1), mask=qa_mask
            )
            loss += nll_loss(
            ...
            _yesno = (
                span_yesno_logits.view(-1)
                .index_select(0, gold_span_end_loc)
                .view(-1, 3)
            )
            _followup = (
                span_followup_logits.view(-1)
                .index_select(0, gold_span_end_loc)
                .view(-1, 3)
            )
            loss += nll_loss(
                F.log_softmax(_yesno, dim=-1), yesno_list.view(-1), ignore_index=-1
            )
            loss += nll_loss(
                F.log_softmax(_followup, dim=-1),
            ...
            self._span_followup_accuracy(
                _followup, followup_list.view(-1), mask=qa_mask
            )
        ...
        for i in range(batch_size):
            ...
            for per_dialog_query_index, (iid, answer_texts) in enumerate(
                ...
                predicted_span = tuple(
                    best_span_cpu[i * max_qa_count + per_dialog_query_index]
                )
    ...
    def _get_best_span_yesno_followup(
        ...
        max_span_log_prob = [
            util.min_value_of_dtype(span_start_logits.dtype)
        ] * batch_size
