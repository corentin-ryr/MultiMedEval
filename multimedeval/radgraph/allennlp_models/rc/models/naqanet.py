...
from multimedeval.radgraph.allennlp.modules import Highway
from multimedeval.radgraph.allennlp.nn.activations import Activation
from multimedeval.radgraph.allennlp.modules.feedforward import FeedForward
from multimedeval.radgraph.allennlp.modules import Seq2SeqEncoder, TextFieldEmbedder
from multimedeval.radgraph.allennlp.modules.matrix_attention.matrix_attention import (
    MatrixAttention,
)
...
from multimedeval.radgraph.allennlp.nn.util import masked_softmax

from multimedeval.radgraph.allennlp_models.rc.models.utils import (
    get_best_span,
    replace_masked_values_with_big_negative_number,
)
from multimedeval.radgraph.allennlp_models.rc.metrics.drop_em_and_f1 import DropEmAndF1
...
class NumericallyAugmentedQaNet(Model):
    ...
    def __init__(
        ...
        self._modeling_proj_layer = torch.nn.Linear(
            encoding_out_dim * 4, modeling_in_dim
        )
    ...
    def forward(  # type: ignore
        ...
        question_weights = self._question_weights_predictor(encoded_question).squeeze(
            -1
        )
        ...
        if "passage_span_extraction" in self.answering_abilities:
            ...
            best_passage_span_log_prob = (
                best_passage_start_log_probs + best_passage_end_log_probs
            )
            if len(self.answering_abilities) > 1:
