...
from multimedeval.radgraph.allennlp.models.model import (
    Model,
...
from multimedeval.radgraph.allennlp_models.rc.models.bidaf import (
    BidirectionalAttentionFlow,
)
from multimedeval.radgraph.allennlp_models.rc.models.utils import get_best_span
...
def ensemble(subresults: List[Dict[str, torch.Tensor]]) -> torch.Tensor:
    ...
    span_start_probs = sum(
        subresult["span_start_probs"] for subresult in subresults
    ) / len(subresults)
    span_end_probs = sum(subresult["span_end_probs"] for subresult in subresults) / len(
        subresults
    )
