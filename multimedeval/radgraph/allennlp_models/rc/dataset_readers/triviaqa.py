...
from multimedeval.radgraph.allennlp_models.rc.dataset_readers import utils
...
class TriviaQaReader(DatasetReader):
    ...
    def _read(self, file_path: str):
        ...
        if "unfiltered" in file_path:
            logger.info(
                "Opening unfiltered tarball file at %s", self._unfiltered_tarball_path
            )
            ...
            data_json = json.loads(
                unfiltered_tarball.extractfile(file_path).read().decode("utf-8")
            )
        else:
            logger.info("Loading question file from tarball")
        ...
        for question_json in data_json["Data"]:
            ...
            for paragraph in self.pick_paragraphs(
                evidence_files, question_text, answer_texts
            ):
                paragraph_tokens = self._tokenizer.tokenize(paragraph)
                token_spans = utils.find_valid_answer_spans(
