...
from multimedeval.radgraph.allennlp.data.tokenizers import (
    Token,
    Tokenizer,
    SpacyTokenizer,
)

from multimedeval.radgraph.allennlp_models.rc.dataset_readers.utils import (
    IGNORED_TOKENS,
    STRIPPED_CHARACTERS,
...
class DropReader(DatasetReader):
    ...
    def text_to_instance(
        ...
        for answer_text in answer_texts:
            ...
            tokenized_answer_texts.append(
                " ".join(token.text for token in answer_tokens)
            )
        ...
        if self.instance_format == "squad":
            ...
            question_concat_passage_tokens = (
                question_tokens + [Token("[SEP]")] + passage_tokens
            )
            valid_passage_spans = []
    ...
    def make_marginal_drop_instance(
        ...
        question_offsets = [
            (token.idx, token.idx + len(token.text)) for token in question_tokens
        ]
        ...
        if answer_info:
            ...
            for signs_for_one_add_sub_expression in answer_info[
                ...
                add_sub_signs_field.append(
                    SequenceLabelField(
                        signs_for_one_add_sub_expression, numbers_in_passage_field
                    )
                )
            if not add_sub_signs_field:
                add_sub_signs_field.append(
                    SequenceLabelField(
                        [0] * len(number_tokens), numbers_in_passage_field
                    )
                )
    ...
    def make_bert_drop_instance(
        ...
        fields["passage"] = passage_field
        fields["question"] = question_field
        question_and_passage_field = TextField(
            question_concat_passage_tokens, token_indexers
        )
        ...
        if answer_info:
            ...
            if not passage_span_fields:
                passage_span_fields.append(
                    SpanField(-1, -1, question_and_passage_field)
                )
            fields["answer_as_passage_spans"] = ListField(passage_span_fields)
