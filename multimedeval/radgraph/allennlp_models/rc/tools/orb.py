...
import argparse
import json
from multimedeval.radgraph.allennlp_models.rc.tools.orb_utils import evaluate_dataset
    ...
    for dataset, metric_dict in metrics.items():
        for metric_name, metric_value in metric_dict.items():
            if metric_name != "total":
                processed_metrics["{0}_{1}".format(dataset, metric_name)] = round(
                    metric_value, 4
                )
