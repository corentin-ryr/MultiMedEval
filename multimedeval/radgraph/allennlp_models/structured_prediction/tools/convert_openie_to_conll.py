...
from multimedeval.radgraph.allennlp.data import Token
from multimedeval.radgraph.allennlp.data.tokenizers import SpacyTokenizer
...
def element_from_span(span: List[Token], span_type: str) -> Element:
    ...
    return Element(
        span_type,
        [span[0].idx, span[-1].idx + len(span[-1])],
        " ".join(str(t) for t in span),
    )
...
def parse_element(raw_element: str) -> List[Element]:
    ...
    elements = (
        regex.match(
            r"^(([a-zA-Z]+)\(([^;]+),List\(([^;]*)\)\))$", elem.lstrip().rstrip()
        )  # noqa
...
def convert_sent_to_conll(sent_ls: List[Extraction]) -> Iterable[Tuple[str, ...]]:
    ...
    return safe_zip(
        *[(str(i) for i in range(len(toks))), toks]
