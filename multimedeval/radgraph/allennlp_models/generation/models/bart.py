...
from multimedeval.radgraph.allennlp.nn.beam_search import BeamSearch
from multimedeval.radgraph.allennlp.nn.util import sequence_cross_entropy_with_logits
from multimedeval.radgraph.allennlp.training.metrics import ROUGE, BLEU
    ...
    def take_step(
        ...
        for i in range(padding_size, last_predictions.shape[1]):
            encoder_outputs = (
                ...
                if state["encoder_states"] is not None
                else None
            )
            outputs = self.bart(
            ...
            if log_probabilities is None:
                ...
                log_probabilities = (
                    decoder_log_probabilities
                    + log_probabilities.gather(dim=-1, index=idx)
                )
