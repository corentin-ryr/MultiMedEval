...
class CNNDailyMailDatasetReader(DatasetReader):
    ...
    def _read(self, file_path: str):
        ...
        cnn_stories = {
            Path(s).stem for s in glob.glob(os.path.join(cnn_stories_path, "*.story"))
        }
        dm_stories = {
            Path(s).stem for s in glob.glob(os.path.join(dm_stories_path, "*.story"))
        }
    ...
    def text_to_instance(
        ...
        if (
            self._source_max_tokens is not None
