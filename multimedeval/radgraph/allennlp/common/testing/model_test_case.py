...
from multimedeval.radgraph.allennlp.commands.train import train_model_from_file
...
from multimedeval.radgraph.allennlp.models import load_archive, Model
from multimedeval.radgraph.allennlp.training import GradientDescentTrainer
    ...
    def ensure_model_can_train_save_and_load(
        ...
        if metric_to_check is not None:
            ...
            metric_value = metrics.get(
                f"best_validation_{metric_to_check}"
            ) or metrics.get(f"training_{metric_to_check}")
            assert (
                metric_value is not None
            ), f"Cannot find {metric_to_check} in metrics.json file"
            assert (
                metric_terminal_value is not None
            ), "Please specify metric terminal value"
        ...
        data_loader = DataLoader.from_params(
            dataset=model_dataset, params=data_loader_params
        )
        data_loader2 = DataLoader.from_params(
            dataset=loaded_dataset, params=data_loader_params2
        )
