...
from multimedeval.radgraph.allennlp.common import Tqdm, util
from multimedeval.radgraph.allennlp.common.checks import ConfigurationError
from multimedeval.radgraph.allennlp.common.file_utils import CacheFile
    ...
    def read(
        ...
        if lazy:
            ...
            if cache_file is not None and not os.path.exists(cache_file):
                ...
                elif util.is_distributed() or (
                    get_worker_info() and get_worker_info().num_workers
                ):
    ...
    def _instance_iterator(self, file_path: str) -> Iterable[Instance]:
        ...
        if cache_file is not None and os.path.exists(cache_file):
            ...
            try:
                ...
                yield from self._multi_worker_islice(
                    self._read(file_path), ensure_lazy=True
                )
                ...
                try:
                    with FileLock(
                        ...
                        with CacheFile(cache_file, mode="w+") as cache_handle:
                            logger.info(
                                "Caching instances to temp file %s", cache_handle.name
                            )
