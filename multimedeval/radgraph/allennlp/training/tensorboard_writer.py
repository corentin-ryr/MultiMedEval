...
from multimedeval.radgraph.allennlp.common.from_params import FromParams
        ...
        if self._train_log is not None:
            if isinstance(values, torch.Tensor):
                ...
                self._train_log.add_histogram(
                    name, values_to_write, self.get_batch_num_total()
                )
    ...
    def log_gradient_updates(
        ...
        for name, param in model.named_parameters():
            ...
            self.add_train_scalar(
                ...
                update_norm
                / (param_norm + nn_util.tiny_value_of_dtype(param_norm.dtype)),
            )
