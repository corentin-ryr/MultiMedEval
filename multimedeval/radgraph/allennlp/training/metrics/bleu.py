...
from multimedeval.radgraph.allennlp.common.util import is_distributed
...
class BLEU(Metric):
    ...
    def _get_modified_precision_counts(
        ...
        clipped_matches = 0
        total_predicted = 0
        from multimedeval.radgraph.allennlp.training.util import ngrams

        for predicted_row, reference_row in zip(predicted_tokens, reference_tokens):
            predicted_ngram_counts = ngrams(
                predicted_row, ngram_size, self._exclude_indices
            )
            reference_ngram_counts = ngrams(
    ...
    def __call__(
        ...
        if not self._exclude_indices:
            ...
            valid_predictions_mask = get_valid_tokens_mask(
                predictions, self._exclude_indices
            )
            valid_gold_targets_mask = get_valid_tokens_mask(
