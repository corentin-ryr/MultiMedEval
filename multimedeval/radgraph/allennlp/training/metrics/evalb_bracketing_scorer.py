...
class EvalbBracketingScorer(Metric):
    ...
    def __call__(self, predicted_trees: List[Tree], gold_trees: List[Tree]) -> None:  # type: ignore
        ...
        if not os.path.exists(self._evalb_program_path):
            ...
            if not os.path.exists(self._evalb_program_path):
                compile_command = (
                    f"python -c 'from multimedeval.radgraph.allennlp.training.metrics import EvalbBracketingScorer; "
        ...
        if is_distributed():
            ...
            correct_predicted_brackets = torch.tensor(_correct_predicted_brackets).to(
                device
            )
